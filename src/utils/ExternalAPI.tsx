...
type OdooDomain = Array<Array<string | number | boolean | null>>;
...
const fetchTransactionsByPartnerId = (
  uid: number,
  partnerId: number
): Promise<any[]> => {
  return new Promise((resolve, reject) => {
...
const fetchAllBranch = (uid: number, branchId: number): Promise<any[]> => {
  return new Promise((resolve, reject) => {
    const client = xmlrpc.createClient({
      url: `/xmlrpc/2/object`,
      headers: { "Access-Control-Allow-Origin": "*" },
...
// Fungsi untuk mengambil data mesin
const fetchAllMachine = (uid: number): Promise<any[]> => {
  return new Promise((resolve, reject) => {
    const client = xmlrpc.createClient({ url: `/xmlrpc/2/object` });
    client.methodCall(
      "execute_kw",
      [
        ...
        "sgeede.mqtt.device",
        "search_read",
        [[["api_control_id", "!=", false]]],
        {
          fields: ["machine_id", "api_control_id"],
        },
      ],
      (error, value) => {
        if (error) reject(error);
  ...
  return new Promise((resolve, reject) => {
    ...
    client.methodCall(
      ...
      async (error, promos) => {
        ...
        const activeVoucher =
          promo.voucher || promo.is_qty_promo || promo.is_tp_voucher;
    ...
    client.methodCall(
      ...
      (error, vouchers) => {
        if (error) {
          ...
          const dataPromo = vouchers.map((voucher: Voucher) => ({
            // Use the Voucher type here
            id: voucher.id,
            ...
            expired: voucher.expired_date
              ? voucher.expired_date.split("T")[0]
              : "-",
            for: `${voucher.for_washer_voucher ? "washer" : ""}${
              voucher.for_dryer_voucher
                ? voucher.for_washer_voucher
                  ? ",dryer"
                  : "dryer"
                : ""
            }`,
      ...
      client.methodCall(
        ...
        [
          ...
          [
            [machineId],
            {
              status_washer: statusWasher,
              status_dryer: statusDryer,
              branch_id: branchId,
            },
          ],
        ],
...
export const runMachine = async (
  ...
  try {
    // Jalankan mesin
    const machineResponse = await statusMachine(
      machineId,
      branchId,
      statusWasher,
      statusDryer
...
export const fetchAllMachineData = async (): Promise<any> => {
  try {
    const uid = await authenticate();
    if (uid) {
      // Ambil data mesin
      const machines = await fetchAllMachine(uid);
...
export const fetchProductsByBranch = async (
  branchId: number
): Promise<any[]> => {
  try {
    const uid = await authenticate();
...
export const checkUserInOdoo = async (
  phoneNumber: string
): Promise<{ partners: any; banned: boolean }> => {
  try {
    const uid = await authenticate();
    if (uid) {
...
export const bypassFetchTransactions = async (
  branchId?: number
): Promise<any[]> => {
  try {
    const uid = await authenticate();
...
export const fetchAllBranchesDatas = async (): Promise<{
  branches: BranchDataModel[];
}> => {
  try {
    const uid = await authenticate();
    if (uid) {
      ...
      // Ambil data karyawan dan teknisi
      const branches = await fetchAllBranch(uid, branchId);

      console.log("branch count:", branches.length);

      // Log detail untuk debugging
      console.log("First few branches:", branches.slice(0, 5));
